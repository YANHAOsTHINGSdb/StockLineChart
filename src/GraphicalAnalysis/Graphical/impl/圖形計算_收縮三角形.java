...
public class 圖形計算_收縮三角形  implements 圖形計算{
	...
	private List<圖形> 取得收縮三角形信息( List<折点> 折點list_優化後) {
		...
		// 每次取得 三个低点(含两个高点)
		int i开始index = 0;
		List<折点> 三个低点list = 平台Util2.取得三个低点(折點list_優化後, i开始index);
		int i狀態 = 0; // 建立三角形
		while(三个低点list != null) {
			...
			if(i狀態 == 0) { // 尋找三角形
				...
				if(b1 && b2) {
					...
					i开始index++;
					三个低点list = 平台Util2.取得三个低点2(折點list_優化後, i开始index, 三个低点list);
					continue;
				...
				if(b1 == false) { // 没有打破三角形
					...
					// t.setI類型(i狀態);// 10=高臺M頭  11=高臺頭肩頂   20=收縮三角形
					 ...
					 折点 z打破折点 = 三个低点list.get(三个低点list.size()-1);
					 三个低点list.remove(三个低点list.size()-1);// 最后一个折点打破三角形不算在内
			...
			// 未找到三角形
			i开始index++;
			三个低点list = 平台Util2.取得三个低点(折點list_優化後, i开始index);
		}
